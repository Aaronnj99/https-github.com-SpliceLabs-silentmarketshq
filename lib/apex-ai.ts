...
export async function chatWithApex(messages: Message[], context: ApexContext): Promise<string> {
...
export async function analyzePortfolio(context: ApexContext): Promise<string> {
  const response = await client.messages.create({
    ...
    max_tokens: 512,
    system: buildSystemPrompt(context),
    messages: [{ role: 'user', content: 'Give me a concise 3-bullet market briefing. Focus on actionable insights. Be direct.' }],
  })
  const block = response.content[0]
  return block.type === 'text' ? block.text : ''
}
