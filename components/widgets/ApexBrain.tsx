...
export function ApexBrain() {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      setMessages([
        ...nextMessages,
        { role: 'assistant', content: 'Connection error. Check your ANTHROPIC_API_KEY.' },
      ])
    } finally {
      setLoading(false)
    }
      ...
      {(brief || briefError) && (
        <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--cyan-dim)', background: 'rgba(0,212,255,0.03)' }}>
          {briefError ? (
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: '12px', margin: 0, lineHeight: '1.6', whiteSpace: 'pre-wrap' }}>
              {brief}
            </p>
          )}
        </div>
